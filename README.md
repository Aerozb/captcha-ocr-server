...
1. `[1] 一键安装或更新（Python + OCR 依赖）`
2. `[6] 部署自检`
3. `[2] 后台启动服务`
4. `[7] 安装开机自启`（可选，但推荐）

每项任务执行完都会在当前窗口显示成功或失败，按回车就会回到主菜单，可以继续执行下一项。菜单顶部还会显示服务、虚拟环境和开机自启的当前状态，并会区分正常、残留、损坏和版本不兼容的 `.venv`。

选项 `[1]` 是完整的一键安装入口：系统已有 Python 3.10-3.12 时直接使用；只装了 Python 3.13/3.14 或尚未安装 Python 时，会从 Python 官网下载并静默安装 Python 3.12.10，然后自动重建 `.venv` 和安装 OCR 依赖。
...
- Windows 10/11
- 运行时使用 Python 3.10-3.12（推荐 Python 3.12）
- 首次安装需要网络，用于下载 Python 安装包（仅在缺少兼容版本时）以及 PyPI 依赖

新系统无需预先配置 Python。管理菜单选项 `[1]` 会自动处理；Python 3.13 及以上不会被选作 OCR 运行时。

## 命令行安装（进阶）

```powershell
# 1. 一键准备兼容 Python、重建无效 venv 并安装 OCR 依赖
...
计划任务里存的是绝对路径，移动后会失效。在新位置重新选择 `[1] 一键安装或更新`，安装器会识别并重建失效的 `.venv`；随后再选择 `[7] 安装开机自启` 覆盖旧的绝对路径。

**提示“未检测到兼容的 Python”**

更新到最新版后重新选择 `[1]`。新版会自动下载 Python 3.12.10，并对安装包执行固定 SHA-256 校验；下载缓存位于系统临时目录 `captcha-ocr-server-python`。网络中断时会自动重试三次。
